...
<div class="page-title fix"><!--Start Title-->
	<div class="overlay section">
		<h2>КомбКомбопідсилювачі для акустики</h2>
	</div>
</div><!--End Title-->
		...
		<div class="row">
			<div class="col-md-3">
				<div class="shop-sidebar fix">
					...
					<h2>Рівень комбіку</h2>
						<ul>
						    <li><a class="activer_c1" values = "nachalo"> Для початковців</a></li>
							<li><a class="activer_c1" values = "prakt"> Для практикуючих</a></li>
							<li><a class="activer_c1" values = "concert"> Для концертів</a></li>
					...
					<h2>Фірма</h2>
						<ul>
						    <li><a 	class="activer_c2" values = "ya"  >Yamaha</a></li>
							<li><a  class="activer_c2" values = "lan" >Laney</a></li>
							<li><a  class="activer_c2" values = "cor" >Cort </a></li>
							<li><a  class="activer_c2" values = "ibanez">IBANEZ</a></li>
							<li><a  class="activer_c2" values = "mar">Marshall</a></li>
							<li><a  class="activer_c2" values = "fen">Fender</a></li>				
						</ul>
					</div>
				</div>
			</div>
			<div class="col-md-9">
				<div class="row">
					<div class="shop-tool-bar col-sm-12 fix">
						...
						<?php
									$products = mysqli_query($link, "SELECT * FROM `combo` WHERE id_vid=2");
						...
						<span>Сортування гітар по: </span>
							<select name="sort-by" class= "sortirvka_4">
						...
						<div class="show-product">
							<span>Вид комбіку:</span>
							<select  name="sort-by" class = "activer_c21">
								<option value = "all_s"> Усі комбіки</option>
								<option value = "lamp">  Ламповий</option>
								<option value = "trans"> Транзисторний</option>
						...
						<?php	
						if($kol==1 )
						echo '<span> В каталозі ' .$kol. ' комбік</span>';
						if($kol==2 ||  $kol==3 || $kol==4)
						echo '<span> В каталозі ' .$kol. ' комбіки</span>';
						if($kol>4)
						
						echo '<span> В каталозі ' .$kol. ' комбіків</span>';
							?>
						</div>
					</div>
					<div class = "git_c1">
					...
					<?php
									...
									$quary = "SELECT * FROM `combo` WHERE id_vid=2 LIMIT $from, $notesOnPage";
									...
									<a class="pro-image fix" href="/combik_details.php?id=<?=$row['c_id']?>"> <img src="<?=$row['c_photo']?>" alt="product" width="270px" height="246px"/></a>
										<!-- Product action Btn -->
										</div>
									
										<!-- Product Name -->
										<div class="pro-name">
											<a href="/combik_details.php?id=<?=$row['c_id']?>"><?=$row['c_name']?></a>
											
												<p><span class="old"><?=$row['c_country']?></span></p>
									...
									echo '</div>';
									$quary = "SELECT COUNT(*) as count FROM `combo` WHERE id_vid=2";
									$table = mysqli_query($link, $quary);
